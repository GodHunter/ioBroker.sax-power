...
var import_strategyChargeReserve = require("./strategyChargeReserve");
var import_strategyChargingDecision = require("./strategyChargingDecision");
var import_strategyChargingInputGrace = require("./strategyChargingInputGrace");
...
const MAXIMUM_ACCEPTANCE_LEARNING_AGE_MS = 12e4;
const POWER_ACCEPTANCE_ROOT = "summary.battery.powerAcceptance";
const recentStableTargetsByAdapter = /* @__PURE__ */ new WeakMap();
function recentStableTargets(adapter) {
...
async function readEstablishedChargeAcceptancePowerW(adapter, createdAt) {
  try {
    ...
    const expected = expectedState == null ? void 0 : expectedState.val;
    const confidence = confidenceState == null ? void 0 : confidenceState.val;
    const lastUpdate = lastUpdateState == null ? void 0 : lastUpdateState.val;
...
async function applyChargePowerTarget(adapter, configuration, contract, publication, currentSocPercent = null) {
  ...
  const learnedAcceptancePowerW = reserveEnabled && currentSocPercent !== null && currentSocPercent < 100 ? await readEstablishedChargeAcceptancePowerW(adapter, publication.lastUpdate) : null;
  const reserve = (0, import_strategyChargeReserve.createStrategyChargeReserve)(
  ...
  const targetChargePowerW = currentSocPercent !== null && currentSocPercent >= 100 ? 0 : reserve.strategyRequestedChargePowerW;
  ...
  return Object.freeze({
    ...
    energyRequiredWh: publication.energyRequiredWh,
...
async function executeStrategyIoBrokerAutomaticChargingCycle(adapter, configuration, contract = import_strategyIntegrationContract.STRATEGY_INTEGRATION_CONTRACT, resolverOptions = {}) {
  ...
  if (!resolution.strategyInputsReady) {
    const graceTarget = (0, import_strategyChargingInputGrace.selectStrategyChargingInputGraceTarget)(
      (_b = recentStableTargets(adapter).get(contract.modbus.chargePowerCommand.stateId)) != null ? _b : null,
      createdAt,
      configuration.maximumChargePowerW
    );
    return applyChargePowerTarget(
      ...
      fallbackPublication(
        ...
        "inputs-not-ready",
        null,
        graceTarget != null ? graceTarget : configuration.maximumChargePowerW
      ),
  ...
  const publication = (0, import_strategyChargingStates.strategyChargingPublicationFromDecision)(decision, createdAt);
  ...
  return applyChargePowerTarget(adapter, configuration, contract, publication, stateOfChargePercent);
}

...
export class BatteryDischargeLoadStateEngine {
	...
	private async publish(root: string, result: BatteryDischargeLoadResult, includeProgress: boolean): Promise<void> {
		const values: Record<string, string | number | boolean | null> = {
			...
			lifetimeTrackingStartedAt: result.progress.lifetimeTrackingStartedAt ?? "",
			...
			totalHighLoadMinutes: Math.round(result.progress.totalHighLoadDurationMs / 6000) / 10,
	...
	private async publishSummary(results: readonly BatteryDischargeLoadResult[]): Promise<void> {
		...
		await Promise.all([
			...
			this.adapter.setStateAsync(`${root}.totalHighLoadMinutes`, { val: total((r) => r.progress.totalHighLoadDurationMs / 60_000), ack: true }),

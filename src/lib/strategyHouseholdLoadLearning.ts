...
export function createEmptyStrategyHouseholdLoadSlot(
	dayClass: StrategyHouseholdDayClass,
	slotIndex: number,
): StrategyHouseholdLoadSlot {
...
export function addStrategyHouseholdLoadSample(
	slot: StrategyHouseholdLoadSlot,
	sample: StrategyHouseholdLoadSample,
): StrategyHouseholdLoadSlot {
	if (
		!Number.isFinite(sample.timestampMs)
		|| !Number.isFinite(sample.averagePowerW)
		|| sample.averagePowerW < 0
	) {
		return slot;
	}
	...
	if (
		resolveStrategyHouseholdDayClass(date) !== slot.dayClass
		|| resolveStrategyHouseholdLoadSlotIndex(date) !== slot.slotIndex
	) {
		return slot;
	}

	const energyWh = sample.averagePowerW * HOUSEHOLD_LOAD_SLOT_MINUTES / 60;
	const samplesWh = [...slot.samplesWh, roundEnergy(energyWh)]
		.slice(-HOUSEHOLD_LOAD_MAX_SAMPLES_PER_SLOT);
...
export function estimateStrategyHouseholdLoad(
	slot: StrategyHouseholdLoadSlot,
): StrategyHouseholdLoadEstimate {
	...
	// Use a conservative expectation once enough observations exist. Until then,
	// the mean avoids over-interpreting a very small sample set.
...
export function estimateRemainingStrategyHouseholdEnergyWh(
	slots: readonly StrategyHouseholdLoadSlot[],
	from: Date,
	until: Date,
): number {
	...
	for (const slot of slots) {
		if (
			slot.dayClass === dayClass
			&& slot.slotIndex >= firstSlot
			&& slot.slotIndex <= lastSlot
		) {
